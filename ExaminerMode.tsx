...
export function ExaminerMode() {
  ...
  const handleEvaluate = async () => {
    ...
    try {
      ...
      // Send a structured intent so server can create a strict evaluation prompt
      const response = await askGemini({ module: 'examiner', subject, input: `Question: ${question || 'Not provided'}\nAnswer: ${answer}` }, token);
      
      const text = response.text;
      if (text && typeof text === 'string' && text.includes('AI features require sign-in')) {
        alert('Your session has expired. Please sign in again.');
        try { localStorage.removeItem('studyDoc_user'); localStorage.removeItem('token'); } catch (e) { /* ignore */ }
        return window.location.reload();
      }
      
      // Parse the text response into the result object
      const scoreMatch = text.match(/Score:\s*(\d+)\/10/i);
      const strengthsMatch = text.match(/Strengths:\s*([\s\S]*?)(?=Weaknesses:|$)/i);
      const weaknessesMatch = text.match(/Weaknesses:\s*([\s\S]*?)(?=How to Improve:|$)/i);
      const suggestionsMatch = text.match(/How to Improve:\s*([\s\S]*?)(?=Encouragement:|$)/i);
      const encouragementMatch = text.match(/Encouragement:\s*([\s\S]*?)$/i);
      ...
      const evaluation: EvaluationResult = {
        ...
        weakAreas: parseList(weaknessesMatch ? weaknessesMatch[1] : ''),
        suggestions: parseList(suggestionsMatch ? suggestionsMatch[1] : ''),
        encouragement: encouragementMatch ? encouragementMatch[1].trim().replace(/^-\s*/, '') : ''
      };
